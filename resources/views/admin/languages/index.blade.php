                ...
                <table class="table table-bordered table-hover table-condensed mb-4">
                    ...
                    <tbody>
                        @foreach ($languages as $item)
                            <tr>
                                <td><img src="{{ url('storage/' . $item->icon) }}" width="30"></td>
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->code }}</td>
                                <td class="text-center">
                                    <ul class="table-controls">
                                        <li><a href="{{ url('admin/languages/' . $item->id) }}" data-toggle="tooltip"
                                        ...
                                        <li>
                                            <form
                                                action="{{ route('admin.languages.destroy', ['language' => $item->id]) }}"method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
